...
def progress(counts):
    ...
    # Rescale once done
    goals = dict(words=42192, pages=278, figures=160)
...
def main():
    ...
    # if it doesn't build!
    error = subprocess.call(['latexmk', '-xelatex', "-latexoption='-interaction=nonstopmode'", 'kamdin_thesis.tex'])
    if error:
        raise ValueError('latexmk failed, check for errors!')

    # Get the counts and time
    out, _ = subprocess.Popen(['texcount', '-sum', '-total', '-merge', 'kamdin_thesis.tex'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
    wc = int(re.search(b'Words in text: ([0-9]+)',out).group(1))
    fc = int(re.search(b'Number of floats/tables/figures: ([0-9]+)',out).group(1))

    out, _ = subprocess.Popen(['pdfinfo', 'kamdin_thesis.pdf'], stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
